...
function getSetting() {
  ...
  let settings = dialogs.multiChoice(
    '任务设置',
    [
...
function registerKey() {
  try {
    ...
    console.log(
      '监听音量键停止失败，应该是无障碍权限出错，请关闭软件后台任务重新运行。'
    )
...
function getCoin() {
  let anchor = className('android.view.View')
    .filter(function (w) {
      if (
        (w.desc() && w.desc().match(/分红：.*份/)) ||
        (w.text() && w.text().match(/分红：.*份/))
      ) {
        return true
      } else {
        return false
...
function openTaskList() {
  ...
  if (!findTextDescMatchesTimeout(/累计任务奖励/, 10000)) {
    ...
    console.log(
      '如果已经打开而未检测到，请删除101版本及以上的webview或使用国内应用市场版京东尝试。'
    )
    quit()
  }
}
...
function getTaskByText() {
  let tButton = null,
    ...
    tTitle = null
  console.log('寻找未完成任务...')
  let taskButtons = textMatches(
    /.*浏览并关注.*|.*浏览.*s.*|.*累计浏览.*|.*浏览可得.*|.*逛晚会.*|.*品牌墙.*|.*打卡.*/
  ).find()
...
function timeTask() {
  ...
  while (c < 40) {
    // 0.5 * 40 = 20 秒，防止死循环
    if (
      textMatches(/获得.*?金币/).exists() ||
      descMatches(/获得.*?金币/).exists()
    )
      // 等待已完成出现
      break
    if (textMatches(/已浏览/).exists() || descMatches(/已浏览/).exists()) {
      // 失败
...
// 入会任务
function joinTask() {
  let check = textMatches(
    /.*确认授权即同意.*|.*我的特权.*|.*立即开卡.*|.*解锁全部会员福利.*/
  ).findOne(8000)
  if (!check) {
    ...
    if (float.length > 1) {
      ...
      if (device.sdkInt >= 24) {
        ...
        console.log(
          '安卓版本低，无法自动移开浮窗，入会任务失败。至少需要安卓7.0。'
        )
        return false
      }
    } else {
...
function shopTask() {
  ...
  sleep(2000)
  console.log('进行', c, '次')
  let like = textContains('喜欢')
    .boundsInside(1, 0, device.width, device.height)
    .findOnce()
  if (!like) {
